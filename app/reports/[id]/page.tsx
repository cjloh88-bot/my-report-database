...
export default async function ReportPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const stage = stages.find(s => s.id === report.stage_id); const ownsReport = report.user_id === profile.id || profile.role === "admin"; const editable = profile.role !== "manager" && ownsReport && (report.status === "draft" || report.status === "returned");
  return <><div className="breadcrumb"><Link href="/reports">Reports</Link><span>/</span><Link href={`/projects/${report.project_id}`}>{project?.name}</Link><span>/</span><span>{stage?.name}</span></div>
    <header className="report-heading"><div><div className="row"><StatusBadge status={report.status}/><span className="muted small">{stage?.name}</span></div><h1>{report.title}</h1><p>Submitted by <strong>{report.submitted_by_name || "Unassigned"}</strong>{report.due_date ? ` · Due ${new Date(`${report.due_date}T00:00:00`).toLocaleDateString()}` : ""}</p></div><div className="header-actions">{profile.role !== "manager" && ownsReport && report.status === "draft" && <form action={submitReportAction}><input type="hidden" name="id" value={id}/><SubmitButton>Submit for review</SubmitButton></form>}{profile.role !== "engineer" && report.status === "submitted" && <form action={startReviewAction}><input type="hidden" name="id" value={id}/><SubmitButton>Start review</SubmitButton></form>}</div></header>
