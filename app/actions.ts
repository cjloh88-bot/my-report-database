...
export async function createProjectAction(form: FormData) {
  const actor = await requireRole("engineer");
...
export async function deleteProjectAction(form: FormData) { await requireRole("engineer"); await deleteProject(required(form, "id", "Project")); refresh(); }
...
export async function deleteReportAction(form: FormData) { await requireRole("engineer"); await deleteReport(required(form, "id", "Report")); refresh(); redirect("/reports"); }
export async function submitReportAction(form: FormData) { const actor = await requireRole("engineer"); const id = required(form, "id", "Report"); await submitReport(id, actor); refresh(); revalidatePath(`/reports/${id}`); }
export async function startReviewAction(form: FormData) { const actor = await requireRole("manager"); const id = required(form, "id", "Report"); await startReview(id, actor); refresh(); revalidatePath(`/reports/${id}`); }
export async function commentOnReportAction(form: FormData) { const actor = await requireRole("manager"); const id = required(form, "id", "Report"); await commentOnReport(id, actor, limited(form, "comment", "Comment", 3000)); refresh(); revalidatePath(`/reports/${id}`); }
export async function approveReportAction(form: FormData) { const actor = await requireRole("manager"); const id = required(form, "id", "Report"); await approveReport(id, actor, limited(form, "comment", "Comment", 3000)); refresh(); revalidatePath(`/reports/${id}`); }
export async function returnReportAction(form: FormData) { const actor = await requireRole("manager"); const id = required(form, "id", "Report"); await returnReport(id, actor, limited(form, "comment", "Comment", 3000)); refresh(); revalidatePath(`/reports/${id}`); }
