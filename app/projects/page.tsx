...
export default async function ProjectsPage() {
  ...
  return <><header className="page-header"><div><p className="eyebrow">PORTFOLIO</p><h1>Projects</h1><p>Create an engineering effort, then organize its stage reports.</p></div></header>
    ...
    </div>{profile.role !== "manager" ? <aside className="panel form-panel"><p className="eyebrow">NEW PROJECT</p><h2>Start a project</h2><form action={createProjectAction} className="form-stack"><label>Project name<input name="name" maxLength={120} required placeholder="e.g. Pump Redesign"/></label><label>Description<textarea name="description" maxLength={1000} rows={4} placeholder="Purpose and scope"/></label><SubmitButton>Create project</SubmitButton></form></aside> : <aside className="panel callout"><p className="eyebrow">MANAGER VIEW</p><h2>Portfolio oversight</h2><p>Managers review submitted work. Engineers create and own projects.</p></aside>}</section></>;
}
